...
<head>
    ...
    <title>{{ $category->name }} Videos - Admin</title>
        ...
        <main class="flex-1 lg:ml-64 w-full">
            ...
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="mb-6 lg:mb-8">
                    <a href="{{ route('admin.categories.index') }}" class="inline-flex items-center text-gray-600 hover:text-gray-900 mb-3 sm:mb-4 font-medium text-sm sm:text-base">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                        </svg>
                        Back to Categories
                    </a>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $category->name }} Videos</h1>
                    <p class="text-sm sm:text-base text-gray-600">Manage videos in this category</p>
                </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Created</th>
                                    ...
                                    @foreach($videos as $video)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                                ...
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <div class="sm:hidden mt-1">
                                                            @if($video->is_premium)
                                                                <span class="px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold">Premium</span>
                                                            @else
                                                                <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs font-semibold">Free</span>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                                                <form action="{{ route('admin.categories.destroyVideo', [$category, $video]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this video from {{ $category->name }}?');">
                                                    @csrf
                                                    @method('DELETE')
                ...
                @else
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 sm:p-12 text-center">
                        ...
                        <h3 class="text-lg sm:text-xl font-bold text-gray-900 mb-2">No videos in this category</h3>
                        <p class="text-sm sm:text-base text-gray-600">This category doesn't have any videos yet.</p>
                    </div>
                @endif
            </div>
        </main>
