            ...
            <div class="p-4 lg:p-6">
                ...
                <nav class="space-y-2">
                    ...
                    <div class="pt-4 mt-4 border-t border-gray-200">
                        ...
                        <a href="{{ route('admin.payments.index') }}" class="flex items-center px-3 sm:px-4 py-2 sm:py-3 text-gray-900 bg-gray-100 rounded-lg font-medium text-sm sm:text-base">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 lg:ml-64 w-full">
            ...
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="mb-6 lg:mb-8">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Manage Payments</h1>
                    <p class="text-sm sm:text-base text-gray-600">View payments and activate subscriptions</p>
                </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Amount</th>
                                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                        <th class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Payment ID</th>
                                    ...
                                    @foreach($payments as $payment)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-3 sm:px-6 py-4">
                                                <div class="text-xs sm:text-sm font-medium text-gray-900">{{ $payment->user->name }}</div>
                                                <div class="text-xs sm:text-sm text-gray-500">{{ $payment->user->email }}</div>
                                                <div class="sm:hidden mt-1">
                                                    <div class="text-xs font-semibold text-gray-900">${{ number_format($payment->amount, 2) }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap">
                                                @if($payment->status === 'paid')
                                                    <span class="px-2 sm:px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold">Paid</span>
                                                @else
                                                    <span class="px-2 sm:px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold">{{ ucfirst($payment->status) }}</span>
                                                @endif
                                            </td>
                                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                                <div class="text-xs sm:text-sm text-gray-500 font-mono truncate max-w-xs">{{ $payment->stripe_id ?? 'N/A' }}</div>
                                            </td>
                                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                                                <div class="text-xs sm:text-sm text-gray-500">{{ $payment->created_at->format('M d, Y') }}</div>
                                            </td>
                                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap">
                                                @if($payment->status !== 'paid' || !$payment->user->subscription_active)
                                                    <form action="{{ route('admin.payments.activate', $payment) }}" method="POST" class="inline">
                                                        @csrf
                                                        <button type="submit" 
                                                            onclick="return confirm('Activate subscription for {{ $payment->user->name }}?')"
                                                            class="px-3 sm:px-4 py-1.5 sm:py-2 bg-green-600 text-white rounded-lg font-semibold text-xs sm:text-sm hover:bg-green-700 transition-colors whitespace-nowrap">
                                                            Activate
                                                        </button>
                                                    </form>
                                                @else
                ...
                @else
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 sm:p-12 text-center mb-6 lg:mb-8">
                        <svg class="w-12 h-12 sm:w-16 sm:h-16 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
                        </svg>
                        <h3 class="text-lg sm:text-xl font-bold text-gray-900 mb-2">No payments found</h3>
                        <p class="text-sm sm:text-base text-gray-600">No payment records available.</p>
                    </div>
                @endif
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                        <h2 class="text-lg sm:text-xl font-bold text-gray-900">Users Without Active Subscription</h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($users->where('subscription_active', false) as $user)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-3 sm:px-6 py-4">
                                        ...
                                        <td class="px-3 sm:px-6 py-4 whitespace-nowrap">
                                            <a href="{{ route('admin.subscriptions.index') }}" class="text-indigo-600 hover:text-indigo-900 font-semibold text-xs sm:text-sm">
                                                Manage →
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-3 sm:px-6 py-6 sm:py-8 text-center text-xs sm:text-sm text-gray-500">
